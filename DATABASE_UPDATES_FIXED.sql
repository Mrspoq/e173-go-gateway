-- Database Updates for Multi-Gateway Support (Fixed Version)
-- Execute these to extend the schema
...
('Airtel Nigeria', '+234802%', 1, 0.042, 0.92),
('Glo Nigeria', '+234805%', 1, 0.040, 0.88),
('9mobile Nigeria', '+234809%', 1, 0.048, 0.85)
...
-- Performance indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sip_calls_performance ON sip_calls (created_at DESC, gateway_id, routed_to_ai);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_call_patterns_analysis ON call_patterns (spam_score DESC, total_calls DESC);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_revenue_daily ON revenue_tracking (billing_date DESC, revenue_source);
